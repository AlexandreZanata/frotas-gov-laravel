<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Modelo de PDF: ') }} {{ $pdfTemplate->name }}
        </h2>
    </x-slot>
            ...
            <div x-data='pdfTemplateEditor(@json($pdfTemplate->toArray()))' x-init="init()" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <form id="templateForm" action="{{ route('pdf-templates.update', $pdfTemplate) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            @include('pdf-templates.partials.form')
                            <div class="mt-8 flex justify-center">
                                <x-primary-button type="submit" class="px-10 py-3 text-sm">{{ __('Atualizar Modelo') }}</x-primary-button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        function pdfTemplateEditor(initialData) {
            return {
                ...
                init() {
                    this.templateData.header_image_url = this.templateData.header_image ? `/storage/${this.templateData.header_image}` : '';
                    this.templateData.footer_image_url = this.templateData.footer_image ? `/storage/${this.templateData.footer_image}` : '';
                ...
                removeImage(scope) {
                    ...
                    this.templateData[`${scope}_image`] = null;
                    const input = document.getElementById(`${scope}_image`);
                    if (input) input.value = '';
                    const form = document.getElementById('templateForm');
                    let removeInput = form.querySelector(`input[name="remove_${scope}_image"]`);
                    if (!removeInput) {
                        removeInput = document.createElement('input');
                        removeInput.type = 'hidden'; removeInput.name = `remove_${scope}_image`; form.appendChild(removeInput);
                    }
                    removeInput.value = '1';
                    this.updatePreview(true);
                },
                ...
                updatePreview(forceImmediate = false) {
                    ...
                    this.debounceTimeout = setTimeout(() => {
                        ...
                        // --- CORREÇÃO DO ERRO PUT vs POST ---
                        // Criamos um FormData manualmente para ter controle total sobre o que é enviado.
                        const formData = new FormData();
                        const form = document.getElementById('templateForm');

                        // Iteramos sobre todos os elementos do formulário
                        form.querySelectorAll('input, select, textarea').forEach(el => {
                            // IGNORAMOS o campo _method que causa o conflito
                            if (el.name === '_method' || el.name === '_token') {
                                return;
                            }
                        ...
                        // Adicionamos as imagens existentes caso nenhum novo arquivo tenha sido selecionado
                        formData.append('header_image_existing', this.templateData.header_image || '');
                        formData.append('footer_image_existing', this.templateData.footer_image || '');

                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        fetch(`{{ route('pdf-templates.ajax-preview.update', $pdfTemplate) }}`, {
                            method: 'POST', // Garantimos que o método seja sempre POST
